...
module.exports = async function handler(req, res) {
 ...
 for (const [league, config] of Object.entries(EVENT_LEAGUES)) {
  ...
  // 4. Safety check: if all metrics are 0, data is likely bad — skip
  const totalMetric = matched.reduce((sum, m) => sum + m.metric, 0);
  if (totalMetric === 0) {
   console.log(` ${league}: All metrics are 0 — data likely invalid, preserving existing scores`);
   results[league] = { status: 'skipped', reason: 'all metrics zero — no valid data' };
   continue;
  }

  // 5. Sort: Golf by position ascending (lower = better), others by points descending
