...
try { var url = TMDB_BASE + '/search/person?api_key=' + TMDB_API_KEY + '&query=' + encodeURIComponent(name); var res = await fetch(url); if (!res.ok) return null; var data = await res.json(); if (!data.results || data.results.length === 0) return null; var exact = data.results.find(function(p) { return p.name.toLowerCase() === name.toLowerCase(); }); return exact ? exact.id : data.results[0].id; } catch (err) { console.error(' TMDB search error for "' + name + '":', err.message); return null; }
}
...
try { var url = TMDB_BASE + '/movie/' + tmdbId + '?api_key=' + TMDB_API_KEY; var res = await fetch(url); if (!res.ok) return 0; var data = await res.json(); return (data.revenue || 0) / 1000000; } catch (err) { return 0; }
}

async function getRTScore(title, year) {
try { var url = 'https://www.omdbapi.com/?apikey=' + OMDB_API_KEY + '&t=' + encodeURIComponent(title) + '&y=' + year + '&type=movie'; var res = await fetch(url); if (!res.ok) return 0; var data = await res.json(); if (data.Response === 'False') return 0; var ratings = data.Ratings || []; var rt = ratings.find(function(r) { return r.Source === 'Rotten Tomatoes'; }); return (rt && rt.Value) ? (parseInt(rt.Value.replace('%', '')) || 0) : 0; } catch (err) { return 0; }
}

function formatDate(isoDate) { if (!isoDate) return ''; var p = isoDate.split('-'); return p.length === 3 ? p[1] + '-' + p[2] + '-' + p[0].slice(2) : isoDate; }
...
for (var category of ['Actor', 'Actress']) {
 ...
 var films = await getActorFilms(personId); await delay(250);
...
console.log(' Done!');
return res.status(200).json({ message: 'Actor/Actress box office update complete', season: seasonYear, timestamp: new Date().toISOString(), results: results });
};